...
public class HewanFragment extends Fragment {
    ...
    private void observeData(String filter) {
        if (filter.equals("Semua")) {
            viewModel.getEndemikByKategori("Hewan").observe(getViewLifecycleOwner(), list -> adapter.setData(list));
        } else {
            viewModel.getEndemikByKategoriAndAsal("Hewan", filter).observe(getViewLifecycleOwner(), list -> adapter.setData(list));
        }
    }
}
